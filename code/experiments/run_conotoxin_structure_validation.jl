...
const N_CHAINS = 50    # must match n_chains in run_omega_conotoxin_experiment.jl
...
"""
    chain_representative_sample(seqs, n_chains) -> Vector

Generation writes each chain's post-burn-in, thinned states consecutively, so
truncating to the first N entries samples only the first few chains rather than the
full chain population. Pick one (the last, most-equilibrated) state per chain instead.
"""
function chain_representative_sample(seqs::Vector, n_chains::Int)
    stride, remainder = divrem(length(seqs), n_chains)
    remainder == 0 ||
        error("$(length(seqs)) sequences do not evenly divide into $n_chains chains")
...
function predict_and_score(name::String, sequence::String, source::String,
    ...
    # SA-generated sources are regenerated by upstream pipeline fixes, so the same
    # sequence name (e.g. SA_strong_0031) can carry different content across runs; key
    # their cache by a content hash so a changed sequence gets refolded instead of
    # silently reusing a stale cached PDB. "stored" (natural family) content never
    # changes, so it keeps the plain name and reuses the existing cache untouched.
    pdb_path = if source == "stored"
        joinpath(struct_dir, "$(source)_$(safe_name).pdb")
    else
        content_id = string(hash(sequence); base=16)
        joinpath(struct_dir, "$(source)_$(safe_name)_$(content_id).pdb")
    end
    ...
    if isfile(pdb_path) && filesize(pdb_path) > 100
        ...
        if isempty(pdb_str)
            return (name=name, source=source, plddt=0.0, tmscore=0.0, pdb_path="", success=false)
        end
    ...
    return (name=name, source=source, plddt=plddt, tmscore=tmscore, pdb_path=pdb_path, success=true)
end
...
for (label, filename) in [
    ...
    if isfile(fasta_path)
        ...
        sources[label] = chain_representative_sample(cleaned, N_CHAINS)
        @info "  Loaded $(length(sources[label])) $label sequences (one per chain)"
    else
...
results_df = DataFrame(
    ...
    pdb_path = [r.pdb_path for r in all_results],

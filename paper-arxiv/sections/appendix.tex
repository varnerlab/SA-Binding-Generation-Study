...
under the equal-similarity assumption; in the canonical replicated sweeps, the
largest absolute deviation of mean attention from $f_{\mathrm{eff}}(\rho)$ was
\AttnMaxDevPts{} percentage points (\AttnMaxDevFamily{},
$\rho=\AttnMaxDevRho$; Table~\ref{tab:per-family-rho}). Thus the finite-sample
agreement is close but not exact. The effective number of patterns under

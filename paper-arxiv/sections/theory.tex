...
distribution at $\beta=0$, where the attention weights reduce to
...
For intermediate $\rho$, the operating point $\beta^{*}(\rho)$ is located as the onset of
the entropy crossover of the weighted attention entropy $H_{\vr}(\beta)$ via a
one-dimensional sweep.
\end{remark}

Empirically, $\beta^{*}$ increases monotonically from 4.4 ($\rho = 1$, $K_{\mathrm{eff}} = 99$)
to 9.3 ($\rho = 1000$, $K_{\mathrm{eff}} = 32.1$) in the Kunitz domain. We report this
displacement as an observation. We do not derive it from $K_{\mathrm{eff}}$.

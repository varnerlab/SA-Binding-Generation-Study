...
continuous-time overdamped Langevin diffusion, is given by:
...
The score function of $p_{\vr}(\vxi) \propto \exp(-\beta\,E_{\vr}(\vxi))$ is given by:
\[
...
absorbed into the softmax logits. The resulting Langevin update is given by:
...
$H_{\vr}(\beta)$ of those weights as $\beta$ increases, and we operate at the
...
Multiplicity weighting breaks that shortcut. The biases $\log r_k$ sit in the logits before $\beta$ does
...
Kunitz family it rose monotonically from 4.4 at $\rho=1$ to 9.3 at $\rho=1{,}000$, and
over that same range the effective pattern count
$K_{\mathrm{eff}}(\vr) = (\sum_k r_k)^{2}/\sum_k r_k^{2}$ fell from 99 to 32.1. That
count equals $K$ when the multiplicities are all equal and falls toward
$K_{\mathrm{des}}$ as $\rho\to\infty$, so it summarizes how concentrated the weights
have become, and we report it beside the $\beta^{*}$ shift as a companion observation
rather than derive either from the other. The two are not the same
...
Setting $\rho$ fixes $f_{\mathrm{eff}}$, the share of the equilibrium mixture that sits on
designated components. That share is a statement about the latent state $\vxi$. What we can
count in the output is a different quantity, the fraction of generated sequences that carry the
designated marker after argmax decoding, which we write $f_{\mathrm{obs}}$. Four steps of the
signal chain separate the one from the other:
\begin{equation}\label{eq:signal-chain}
  ...
  f_{\mathrm{obs}},
\end{equation}
where $\bar{a}_{\mathrm{des}}$ is the attention weight summed over the designated patterns and
averaged over the retained states of the sampler (Eq.~\ref{eq:a-des}), and $f_{\mathrm{soft}}$ is the continuous-valued probability of the designated residue
at the marker position, read off before the decoder commits to a letter. The labels say what
each step does to the number handed to it. \emph{Exact} means the step introduces no
approximation of its own. Eq.~\eqref{eq:f-eff} turns $\rho$ into $f_{\mathrm{eff}}$ by algebra,
and the multiplicities enter the attention logits directly in Eq.~\eqref{eq:weighted-ula}, which
is why the measured $\bar{a}_{\mathrm{des}}$ tracks $f_{\mathrm{eff}}$. \emph{Lossy} means the
step discards information it was given. The PCA reconstruction returns a blend of amino acid
frequencies, and the marker distinction can be buried in that blend when designated and
background patterns sit close together in PCA space. \emph{Nonlinear} means the step is a
threshold rather than a proportion. Argmax keeps only the residue with the highest probability,
so a shift of $f_{\mathrm{soft}}$ from 0.45 to 0.55 flips every decoded letter at that position,
while an equal shift from 0.05 to 0.15 flips none. The calibration gap is what the chain loses
end to end, $\Delta = f_{\mathrm{eff}} - f_{\mathrm{obs}}$, and each step contributes a term
...
\end{equation}
The first term asks whether the sampler delivered the mixture it was asked for, the second how
much of the marker distinction survived the representation, and the third how much of what
survived cleared the threshold.

The decomposition locates where the number is lost. It does not say what a matched number would
prove. Because the target is exactly a mixture, we can also describe the final step by
conditioning on which component actually produced the sample.
Let $C$ be that component and let $\mathcal{D}$ be the index set of the designated patterns, so
that $C\in\mathcal{D}$ says the draw came from a designated component. We define:
\[
\begin{aligned}
q_{\mathrm{des}}
...
\end{aligned}
\]
These are decoding rates, one per group. The rate $q_{\mathrm{des}}$ is how often a draw from a
designated component decodes to a marker-positive sequence, which is the outcome we want, and
$q_{\mathrm{bg}}$ is how often a background draw does the same, which is an error. Combining these rates gives:
...
path. Matching $f_{\mathrm{obs}}$ to $f_{\mathrm{eff}}$ in particular does not
...
Both the lossy step and the nonlinear step depend on how the designated and background
sequences sit relative to each other in PCA space, so we need one number for that geometry,
comparable across families.
We define the separation index

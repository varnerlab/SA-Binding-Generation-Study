...
non-Pfam provenance. The attention weights tracked $f_{\mathrm{eff}}$
closely in all four families, indicating that the multiplicity-weighted energy operates as expected
...
We tested whether preserved pharmacophores translate to predicted binding competence using ColabFold AlphaFold2-multimer predictions for SA-generated sequences paired with the Cav2.2 pore-domain target (Table~\ref{tab:docking-validation}). Complex-level scores were similar across groups: SA strong-conditioned ($n=10$; pTM $0.221\pm0.007$, iPTM $0.105\pm0.008$, interface-pLDDT $37.2\pm5.4$), SA full-conditioned ($n=10$; pTM $0.219\pm0.008$, iPTM $0.104\pm0.007$, interface-pLDDT $38.2\pm4.0$), and natural controls ($n=5$; pTM $0.228\pm0.004$, iPTM $0.108\pm0.007$, interface-pLDDT $40.7\pm3.5$). No pairwise comparison reached significance (exact permutation tests; all $p>0.05$). These scores are modest in absolute terms and the sample sizes are small, so the results are best interpreted as consistent with pharmacophore-level plausibility rather than as evidence of binding; larger control sets and experimental validation are needed.

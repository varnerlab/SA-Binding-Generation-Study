...
did; and full-family generation produced 39\% K/R, matching the natural proportion
($32/99 = 32\%$) within sampling noise, which demonstrates that the designated phenotype was
inherited perfectly while the fold-level composition was preserved. To determine the minimum input size required for reliable transfer,
...
multiplicity-weighted generation (Fig.~\ref{fig:rho-sweep},
Fig.~\ref{fig:phase-transition}, Fig.~\ref{fig:gap-decomposition}). For each $\rho$, we
...
monotonically from 0.39 ($\rho=1$) to 0.63 ($\rho=500$), substantially exceeding the natural
...
divergence increased from 0.007 to 0.017 (Fig.~\ref{fig:rho-sweep}). The phase transition
...
reconstruction (Fig.~\ref{fig:gap-decomposition}). These observations suggest that the PCA
...
(Table~\ref{tab:cross-family}, Fig.~\ref{fig:cross-family},
Table~\ref{fig:per-family-rho}). We found that the calibration gap decreased monotonically with
...
$f_{\mathrm{eff}} = 0.999$); a linear fit yielded $\Delta \approx 0.84 - 2.5\,S$ with
$R^{2} \approx 1.0$. We observed that the attention weights tracked $f_{\mathrm{eff}}$
perfectly in all three families, which confirms that the multiplicity-weighted energy is exact
regardless of the downstream calibration gap, while hard curation achieved 100\% phenotype
transfer in all three families regardless of separation index---as expected, since curated
memory matrices are built from designated-subset-only PCA, which by construction captures
subset-specific variation in its leading components (Fig.~\ref{fig:cross-family}). To test
...
(Table~\ref{fig:conotoxin-pharmacophore}, Fig.~\ref{fig:conotoxin-loop}). Cav2.2 is a primary
...
(Table~\ref{fig:conotoxin-pharmacophore}). The basic-residue (Lys + Arg) fraction was also
...
conditioning effect visually: the strong-binder input and strong-binder-generated heatmaps were
...
sequence (Fig.~\ref{fig:conotoxin-loop}). Together, these results confirm that curated memory
conditioning transferred Cav2.2 pharmacophore statistics from a small set of 23 characterized
sequences to a diverse library of 1\,550 novel candidates, with no training and no target
structure required.
...
(Table~\ref{fig:structure-validation}, Table~\ref{fig:baseline-comparison}). We found that
...
threshold; whereas HMM-emitted sequences achieved only $60.4 \pm 13.1$ mean pLDDT (20\% above
threshold) and $0.60 \pm 0.10$ mean TM-score, which suggests that profile HMM emission does not
reliably produce foldable sequences (Table~\ref{fig:structure-validation}). We further assessed
...
(Table~\ref{fig:structure-validation}). We also compared SA against HMMER3 profile HMM emission
...
unavailable to unconditional baselines (Table~\ref{fig:baseline-comparison}). These results

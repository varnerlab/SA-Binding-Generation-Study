...
is vastly over-dimensioned relative to the number of stored patterns $K$
...
95\% of total variance). Across the four families, $d$ ranged from 20
($\omega$-conotoxin) to 186 (WW), yielding compression ratios of
$3.3\times$ to $26\times$ (Table~\ref{tab:cross-family}). Each sequence was
then projected as
...
and all families (Table~\ref{fig:per-family-rho}), validating the
...
$\beta^{*}$ increased from 4.3 ($\rho=1$, $K_{\mathrm{eff}} = 99$) to 10.1
...
Tables~S\ref{tab:kunitz-rho}--S\ref{tab:ww-rho} report selected values from
the $\rho$ sweep on each of the three benchmark families.
...
diversity. Table~S\ref{tab:beta-sweep} reports phenotype fraction and
...
with $\beta^{*}(\rho)$ increasing from 3.85 ($\rho=1$) to 10.22

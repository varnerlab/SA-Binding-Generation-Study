...
algorithm (ULA) with score step size $\alpha/\beta$, for $\alpha \in (0,1)$, yields the stochastic attention
...
The derivation is given in the Supporting Information. Equation~\eqref{eq:gmm-target}
is the exact target of the continuous-time diffusion, not generally the invariant law
of its finite-step Euler discretization. Thus, Eqs.~\eqref{eq:ula} and
\eqref{eq:weighted-ula} below retain step-size and finite-time error at nonzero
$\alpha$~\cite{durmusMoulinesULA2017}.
...
designated fraction, which is the exact latent designated-component probability under the
Gaussian-mixture target, is
...
\end{equation}
For unit-norm memories, $\beta$ changes the common component covariance and therefore
the overlap, posterior responsibilities, and decoded output, but it does not change the
prior mixture weights $w_k$ or $f_{\mathrm{eff}}$.
...
\end{equation}
The exact mixture representation also gives a direct component-conditioned
description of the final step. Let $C$ be the generating mixture component, and define
\[
\begin{aligned}
q_{\mathrm{des}}
...
\begin{equation}\label{eq:decoder-pushforward}
  ...
  - (1-f_{\mathrm{eff}})q_{\mathrm{bg}}.
\end{equation}
Thus $f_{\mathrm{obs}}=f_{\mathrm{eff}}$ is not an analytic requirement at
$\rho=1$; it would require a particular balance of false negatives and false
positives under the decoder. Equation~\ref{eq:decoder-pushforward} makes the
calibration gap an empirical property of the complete latent-to-sequence
pushforward, not a test that selects a unique PCA scaling.
To describe the PCA geometry across families, we define the Fisher separation index

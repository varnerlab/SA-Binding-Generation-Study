...
unit-norm model. We use it as the reference in the exact-equilibrium benchmark. We retain ULA
for the reported generation runs to compare with the original stochastic-attention workflow and
to characterize finite-step behavior. ULA is not required to sample this target.
Equation~\eqref{eq:gmm-target}
...
vector by $\rho = r_{\mathrm{designated}}/r_{\mathrm{background}}$. We set
$r_k = \rho$ for patterns in the user-designated subset and $r_k = 1$ for background patterns.
The effective
...
distribution at $\beta=0$. At this point, the attention weights are
$w_k = r_k / \sum_j r_j$. Their limiting entropy is the Shannon entropy

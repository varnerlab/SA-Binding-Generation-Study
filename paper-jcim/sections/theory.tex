...
unit-norm model, and we use it as the reference in the exact-equilibrium benchmark, while ULA,
which is not required to sample this target, is retained for the reported generation runs to
compare with the original stochastic-attention workflow and to characterize finite-step
behavior. Equation~\eqref{eq:gmm-target}
is the exact target of the continuous-time diffusion, not generally the invariant law
of its finite-step Euler discretization, so Eqs.~\eqref{eq:ula} and
...
vector by $\rho = r_{\mathrm{designated}}/r_{\mathrm{background}}$, setting
...
$f_{\mathrm{eff}} = K_{\mathrm{des}}/K$, and as $\rho \to \infty$, $f_{\mathrm{eff}} \to 1$, so
inverting gives the multiplicity ratio needed for a target fraction $f$:
